...
LOAF_INPUT_FOLDERS = [r"data\loaf_annotations\annotations\resolution_1k"] # Adjust this path as needed
LOAF_TARGET_JSONS = ["instances_train.json"]
LOAF_OUTPUT_DIR = os.path.join(BASE_DIR, "LOAF")
CUSTOM_NUM_WORKERS = 16 # default to 16, can be adjusted based on system capabilities
...
def restructure():
    ...
    if os.path.exists(vis_dst):
        print("Visdrone folder already exists. Removing it.")
...
def merge_fisheye1keval_images():
    print("\n Merging Fisheye1keval images...")

    src_dirs = [
        os.path.join(BASE_DIR, "Fisheye1keval", "images", "images"),
        os.path.join(BASE_DIR, "Fisheye1keval", "images1", "images1")
    ]
    dst_dir = os.path.join(BASE_DIR, "Fisheye1keval", "Merged_images")
    ...
    for src in src_dirs:
        ...
        for filename in os.listdir(src):
            if filename.lower().endswith(('.jpg', '.png', '.jpeg', '.bmp')):
                ...
                dst_filename = f"{counter:06d}_" + filename  # avoid overwrite
                dst_path = os.path.join(dst_dir, dst_filename)
                shutil.copy2(src_path, dst_path)
                counter += 1

    print(f"Merged total {counter} images into {dst_dir}")

if __name__ == "__main__":
    restructure()
    process_visdrone()
    process_loaf_annotations()
    merge_fisheye1keval_images()
